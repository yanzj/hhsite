...
//两次密码输入不一致
  ...
  //本地测试地址
  public fileUploadUrl = environment.server + '/fileLoad/uploadFile';//文件上传
  private serverUrl = environment.server +'/backendapi';//进件本地调试地址
  private serverUrl2 = environment.server +'/backendapi';//询价本地调试地址
  ...
  onJudgeSuccessful(dataOBJ:any){
    ...
    if(result){
      ...
      if(s == "" || s == "#" || s == "/" || s == "#/login" || s == "#/home/main"){
        ...
        this.popNextFailModal("登录超时","登录超时，请重新登录,",true,()=> {
