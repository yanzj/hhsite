...
export class ApplyMaterialComponent implements OnInit {
  ...
  meterialNumberFunc(fielList){
    if(fielList){
      ...
      for(var j=0;j<fielList.length;j++){
        file = [];
        var defaultImgUrl=this.setFileNameCatchImage(fielList[j].originalFileName);  //y原来用是的 judgeFileTypeFunc
  ...
  //下载文件
  downloadFile(file){
    var defaultImgUrl=this.setFileNameCatchImage(file.fileName); //y原来用是的 judgeFileTypeFunc
    if ('needUrl' == defaultImgUrl) {
