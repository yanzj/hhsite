...
export class ApplySectionComponent implements OnInit {
 ...
 //面包屑导航根据点击进来的路径不同显示不同的方法
  ...
  delete(event, no) {
    var className1: any = document.getElementsByClassName("firstNum" + no)[0];
    var className2: any = document.getElementsByClassName("secondNum" + no)[0];
    var className3: any = document.getElementById("mortgage"+no);
    ...
    var guarantee3 = className3.value == '' || null == className3.value || '' == className3.value.trim();
    ...
    //删除一行数据时，应清楚可能存在的提示
    $("#firstNum" + no).removeTip();
    $("#secondNum" + no).removeTip();
    $("#mortgage" + no+'-select2-selection--single').removeTip();
  ...
  Validates = {
    ...
    //检查是否为中文名是否为空
    checkChineseName:(id, myErrorPosition, myMargainLeft, errorText)=>{
      var chinese = /^[\u4e00-\u9fa5]*$/;
      var str = $("#" + id).val();
      if (this.isEmpty(id)) {
        $("#" + id).createTip({
        ...
        return false;
      } else if (chinese.test(str) == false) {
        $("#" + id).createTip({
          margainLeft: myMargainLeft,
          errorText: '格式错误',
          errorPosition: myErrorPosition,//参数包括right,top-right,top-left
  ...
  motorageNum:number=1;
  addCertificateListFunc() {
    this.num.push({'no':this.motorageNum++});
  ...
  ngOnInit() {
    ...
    this.ValidateId("mainName", this.Validates.checkChineseName, 'bottom', this.tipsMarginLeft, "主借款人姓名不能为空");
    ...
    //校验 “放款方式”"产品名称","申请金额"为收据时申请金额不能超过最大给的额度-start
    let _that=this;
    $("#proDuctName").change(()=>{
      if(!this.isEmpty("proDuctName")){
        this.checkApplyMount ();
      }else {
        $("#applyMoney").removeTip();
      }
    });
    ...
    $("#method").change(()=>{
      ...
      if(creditTypeCode== "collection_receipt_loan"){
        this.checkApplyMount ();
      }else {
        $("#applyMoney").removeTip();
      }
    ...
    //校验 “放款方式”"产品名称","申请金额"为收据时申请金额不能超过最大给的额度-end
  ...
  var creditTypeCode = $("#method").val();
  //申请金额的额度
  if(creditTypeCode== "collection_receipt_loan" &&( productValue!=''||productValue!=null)) {
    if (productName!="undefined" && productName != "" && limit > 0) {
      limit = limit - 0;
      applyMony = applyMony - 0;//把字符串变成数数字
      if (applyMony > limit) {
        this.Validates.checkLimit("applyMoney", 'bottom', this.tipsMarginLeft, "不能大于" + limit + "万元");
    ...
    this.networkService.postData(Obj, false).then(res => {
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          this.initMortgageSelectFunc();  //对抵押物信息进行初始化
  ...
  initMortgageSelectFunc(){
    ...
    this.add_option(this.mortgageTypeList, '#mortgage'+this.num[lengthNum]['no'] , ['mortgageTypeCode', 'mortgageTypeName']);
    this.ValidateId("firstNum"+this.num[lengthNum]['no'], this.Validates.checkNull, 'bottom', this.tipsMarginLeft, "不能为空");
    this.ValidateId("secondNum"+this.num[lengthNum]['no'], this.Validates.checkNull, 'bottom', this.tipsMarginLeft, "不能为空");
    this.ValidateIdForSelect2("mortgage"+this.num[lengthNum]['no'], this.Validates.checkNullForSelect2, 'bottom', this.tipsMarginLeft, "不能为空");
    //将抵押物信息添加
    $(".selectmsg").select2({});
  ...
  initResubmitMortgageSelectFunc(){
    for (var i=0;i<this.num.length;i++){
      if(i>0){
        this.add_option(this.mortgageTypeList, '#mortgage'+this.num[i]['no'] , ['mortgageTypeCode', 'mortgageTypeName']);
      }
          this.ValidateId("firstNum"+this.num[i]['no'], this.Validates.checkNull, 'bottom', this.tipsMarginLeft, "不能为空");
          this.ValidateId("secondNum"+this.num[i]['no'], this.Validates.checkNull, 'bottom', this.tipsMarginLeft, "不能为空");
          this.ValidateIdForSelect2("mortgage"+this.num[i]['no'], this.Validates.checkNullForSelect2, 'bottom', this.tipsMarginLeft, "不能为空");
          if(this.mortgageList[i]["certificateNumberFirst"]){
            $("#firstNum"+this.num[i]['no']).val(this.mortgageList[i]["certificateNumberFirst"]);
          }
          if(this.mortgageList[i]["certificateNumberSecond"]){
            $("#secondNum"+this.num[i]['no']).val(this.mortgageList[i]["certificateNumberSecond"]);
          }
          if(this.mortgageList[i]["mortgageTypeCode"]){
            $("#mortgage"+this.num[i]['no']).val(this.mortgageList[i]["mortgageTypeCode"]).trigger("change");
          }
...
ngAfterViewChecked(){
  ...
  getCurrentBusinsessStatuInfo() {
    ...
    if (BusinessData != null) {
      ...
      this.loanSerialNo = BusinessData.loanSerialNo;
      var temporaryObj = {
        'head': {
          'functionNo': 'HH000006',
          'userNo': this.userDataKe.userNo,
      ...
      this.networkService.postData(temporaryObj, false).then(res => {
        if (res.retCode == true) {
          ...
          if (this.networkService.onJudgeSuccessful(res)) {
            ...
            // 2 获取业务经理
            this.agentId = this.commonFunc.handleNilString(userDataObj.agentId);
            // 3 主借款人姓名
            ...
            // 5 产品名称
            this.productCode = this.commonFunc.handleNilString(userDataObj.productCode);
            //1 获取渠道
            ...
            // 7 申请金额
            var loanAmount = userDataObj.loanAmount;
            loanAmount=loanAmount*1/10000;
            ...
            $("#remark").val(remark);
            this.loanSerialNo = this.commonFunc.handleNilString(userDataObj.loanSerialNo);
            ...
            /*****************************修改**************************************/
            this.mortgageList=userDataObj.mortgageList;
            if(userDataObj.mortgageList.length&&userDataObj.mortgageList.length>1){
              for (var i= 1; i<userDataObj.mortgageList.length;i++){
                this.num.push({'no':i});
              }
            }
            //赋值   如果请求到的值不为空
            if(this.mortgageList.length){
              this.isReloadBuinessData = true;
            }

          // 暂存后在申请   将之前暂存的文件显示在页面上
            if(userDataObj.applyFileId){//如果暂存的时候已经暂存了文件
              this.isShow2=true;
              this.applyFileName=userDataObj.applyFileName;
              this.uploadTime=userDataObj.applyTime;
              this.applyFileId=userDataObj.applyFileId;
              this.isFile=true;
            }
      ...
      that.networkService.postData(Obj, false).then(res => {
        if (res.retCode == true) {
          ...
          if (that.networkService.onJudgeSuccessful(res)) {
            ...
            $("#manager").val(that.agentId).trigger("change");
            //添加产品到产品列表
            ...
            var readFlag: string = that.commonFunc.getBusinessSaveFlagFunc();
            if (readFlag == that.commonFunc.businessReadValue) {
              // 当前组件符合读取暂存进件条件
              var maxLoanAmount;
              for (var i=0;i<that.productList.length;i++){
                if (that.productCode==that.productList[i].productCode){
                  maxLoanAmount=that.productList[i].maxLoanAmount;
                }
              }
              $("#proDuctName").val(that.productCode+','+maxLoanAmount).trigger("change");
            }
...
//组件销毁时调用方法
  ...
  uploadFile() {
    ...
    this.isUp = true;
    this.applyFileName = this.uploader.queue[0].file.name;
    //上传完成
    ...
    this.uploader.queue[0].onSuccess = (response, status, headers) => {
      ...
      if (status == 200) {
        ...
        this.applyFileName = tempRes.body.fileMaps[0].fileName;
        this.applyFileId = tempRes.body.fileMaps[0].serialNo;
  ...
  q_submit() {
    ...
    console.log(manager);
    var mainName = this.Validates.checkChineseName("mainName", 'bottom', this.tipsMarginLeft, "主借款人姓名不能为空");
    ...
    console.log(intentionM);

    var flag: boolean = true;
    //校验 “放款方式”为收据时申请金额不能超过最大给的额度
    //产品名称
    var productValue = $("#proDuctName").val();
    var productArray = productValue.split(",");
    var productName = productArray[0];
    ...
    if(creditTypeCode== "collection_receipt_loan" &&( productValue!=''||productValue!=null)) {
      if (productName!="undefined" && productName != "" && limit > 0) {
        limit = limit - 0;
        applyMony = (applyMony - 0)/10000;//把字符串变成数数字
        if (applyMony > limit) {
          this.Validates.checkLimit("applyMoney", 'bottom', this.tipsMarginLeft, "不能大于" + limit + "万元");
          return flag && false;
        }
    ...
    var thirdNum = true;
    for (var i = 0; i < obj; i++) {
      firstNum = this.Validates.checkNull("firstNum" + this.num[i]['no'], 'bottom', this.tipsMarginLeft, "不能为空");
      secondNum = this.Validates.checkNull("secondNum" + this.num[i]['no'], 'bottom', this.tipsMarginLeft, "不能为空");
      thirdNum = this.Validates.checkNullForSelect2("mortgage" + this.num[i]['no'], 'bottom', this.tipsMarginLeft, "不能为空");
    ...
    for (var i = 0; i < obj; i++) {
      mortgageTypeList.push({
        "certificateNumberFirst": $("#firstNum" + this.num[i]['no']).val(),
        "certificateNumberSecond": $("#secondNum" + this.num[i]['no']).val(),
        "mortgageTypeCode": $("#mortgage" +this.num[i]['no']).val()
      })
    }
    ...
    flag = flag && company && manager && mainName && proDuctName && method && applyMoney && deadline && intentionM;
      if (!flag) {
        this.networkService.popNextSuccessModal("进件申请错误","将提交的进件申请存在错误，请修改",false);
  ...
  q_Temporary() {
    ...
    $("#q_Temporary").attr("disabled",true);
    // 暂存不需要验证
    ...
    var mortgageTypeList = [];
    for (var i = 0; i < obj; i++) {
      //如果产证编号 抵押物类型都是空的话 则不传入
      if($("#firstNum" + this.num[i]['no']).val()|| $("#secondNum" +this.num[i]['no']).val()||$("#mortgage" +this.num[i]['no']).val()){
        mortgageTypeList.push({
          "certificateNumberFirst": $("#firstNum" + this.num[i]['no']).val(),
          "certificateNumberSecond": $("#secondNum" +this.num[i]['no']).val(),
          "mortgageTypeCode": $("#mortgage" +this.num[i]['no']).val()
    ...
    if (!this.limitFlag){
      $("#q_Temporary").attr("disabled",false);
      return false;
    }else{
      ...
      this.networkService.postData(Obj, false).then(res => {
        ...
        if (res.retCode == true) {
          ...
          //  暂存之后给相应的serialNo赋值;
            this.loanSerialNo=userDataObj.loanSerialNo;
          }
        }
      })
    }
  ...
  //删除按钮  删除
  deleteFunc(){
    this.networkService.popNextSuccessModal("删除进件申请", `将删除借款人为<span class="main-color font-weight-bold">${this.customerName}</span>的进件申请`, true, () => {
