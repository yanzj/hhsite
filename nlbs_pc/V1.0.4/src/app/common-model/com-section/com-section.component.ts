...
export class ComSectionComponent implements OnInit {
  ...
  muneIconIndex:{} = {mune_icon_index:true};
  muneIconEnquiry:{} = {mune_icon_enquiry:true};
  muneIconItem:{} = {mune_icon_item:true};
  muneIconPersonCenter:{} = {mune_icon_personCenter:true};
  //一级菜单点方法
...
//退出登录end
    ...
    this.networkService.postData(Obj, false).then(res => {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          for(let i=0;i<this.messageTipsList.length;i++){
            ...
            var repaymentDate=internalParam.repaymentDate; //还款日期
            var messageMsgType=internalParam.msgType;
            if(messageMsgType=='001'){  //待评估
            ...
            }else if(messageMsgType=='002'){//已评估
               var messageLink=`<a href="./#/home/enquiryDetail" class="text-link">${messageKeyWords}</a>`
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }else if(messageMsgType=='003'){//正在评估中

            } else if(messageMsgType=='004'){ //还款计划

              if(repaymentDate){
                sessionStorage.setItem("sign","004");
                sessionStorage.setItem("repaymentDate",repaymentDate);
              }
              var messageLink=`<a href="./#/home/repaymentQuery" class="text-link">${messageKeyWords}</a>`
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }else if(messageMsgType=='005'){//逾期计划
              var messageLink=`<a href="./#/home/overdueQuery" class="text-link">${messageKeyWords}</a>`
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }
 ...
 // 侧边栏点击后改变
 //  personCenterToCom(){
 //      var changedMess=sessionStorage.getItem("unreadMessageChange");
 //      if(changedMess=="changed"){
 //       sessionStorage.removeItem("unreadMessageChange");
 //        this.InitMessageList();
 //      }
 //  }
