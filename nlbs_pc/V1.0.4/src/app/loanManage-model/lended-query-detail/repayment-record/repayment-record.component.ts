...
declare var $: any;
@Component({
  selector: 'app-repayment-record',
  templateUrl: './repayment-record.component.html',
  styleUrls: ['./../lended-query-detail.component.css','./repayment-record.component.css']
})
export class RepaymentRecordComponent implements OnInit {
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          //  }
          this.currentItemLists = res.data.body.receiptsRecordInfo.receiptsRecordList;
