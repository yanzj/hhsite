...
@Component({
  selector: 'app-overdue-record',
  templateUrl: './overdue-record.component.html',
  styleUrls: ['./overdue-record.component.css']
})
export class OverdueRecordComponent implements OnInit {
  ...
  getSearchCondigtion() {
    ...
    var distributorCode = this.commonFunc.handleNilString($("#cityList").val());//选择渠道
    var customerName = this.commonFunc.handleNilString($("#loanCustomer").val());//借款人
    var repaymentStartDate = this.commonFunc.handleNilString($("#applyTimeBegin").val()); //还款开始日期
    var repaymentEndDate = this.commonFunc.handleNilString($("#applyTimeEnd").val());     //还款结束日期
    var receipt = this.commonFunc.handleNilString($("#receipt").val()); //收款方
    var overdueDaysStart = this.commonFunc.handleNilString($("#overdueTimeBegin").val());  //业务经理
    var overdueDaysEnd = this.commonFunc.handleNilString($("#overdueTimeBegin").val());  //逾期开始天数
    var agentName = this.commonFunc.handleNilString($("#overdueTimeEnd").val());  //逾期结束天数
  ...
  QueryEntryList=() =>{
    ...
    var Obj = {
      ...
      'body': {
        ...
        'repaymentStartDate': searchConditions.repaymentStartDate,
        'repaymentEndDate': searchConditions.repaymentEndDate,
        "agentName": searchConditions.agentName,
        "overdueDaysStart":searchConditions.overdueDaysStart,
        "overdueDaysEnd":searchConditions.overdueDaysEnd,
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          //   this.currentItemLists[i]["loanStatusName"] = resultObj['loanStatusName'];
          // }
  ...
  onSecondOperationClick(item){
    this.router.navigate(['/home/overdueRecord', item.repaymentScheduleCode]);
