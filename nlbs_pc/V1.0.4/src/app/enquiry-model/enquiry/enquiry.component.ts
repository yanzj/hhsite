...
export class EnquiryComponent implements OnChanges, OnInit {
  ...
  ngOnInit() {
    ...
    var that:any = this;
    // this.contact={
    //   "breadcrumbNavDisplay":false,
    //   "title":"自动询价"
    // };
    this.contact=this.user.setbreadcrumbNav(false,'自动询价');
 }
...
//提交询价信息
  ...
  //别墅评估
  villaAssess() {
    $("#villaAssess").attr("disabled",true);
    ...
    if(!villaFormatResult){
      $("#villaAssess").attr("disabled",false);
      //别墅建筑面积格式错误
