...
InitEnquiryDataFunc() {
  ...
  uploadFile() {
    ...
    this.uploader.queue[0].onSuccess = (response, status, headers) => {
      ...
      if (status == 200) {
        ...
        console.log(this.fileName, this.fileSerialNo, this.isFile);
        that.isShow = !that.isShow;
        that.isShow2 = !that.isShow2;
        console.log(that.isShow2);
 ...
 vaildReadyEnquiryDataCallBack(flag:number){
  ...
  if (flag == 0) {
   ...
   //判断价格输入是否正确
   var priceVaildResult = this.validInputPriceFunc(priceDivID);
   console.log('the num you scanf is that',priceVaildResult);
