...
import com.vilio.pcfs.service.FileService;
...
public class MoblieController {
     ...
     * @param modelMap
     */
    @RequestMapping(value = "/file.f", method = RequestMethod.GET)
    public void file(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        String serialNo = request.getParameter("serialNo");
        String fileName = request.getParameter("fileName");
        FileService fileService = (FileService) SpringContextUtil.getBean("fileService");
        try {
            fileService.doMain(serialNo, fileName, response);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
