...
@Component({
  selector: 'app-repayment-register-query',
  templateUrl: './repayment-register-query.component.html',
  styleUrls: ['./repayment-register-query.component.css']
})
export class RepaymentRegisterQueryComponent implements OnInit {
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          this.add_option(fundSourceList, '#payment', ['fundSourceCode', 'fundSourceName']);
  ...
  getSearchCondigtion() {
    ...
    var accountTypeCode = this.commonFunc.handleNilString($("#receipt").val()); //收款方
    var fundSourceCode = this.commonFunc.handleNilString($("#payment").val()); //还款来源
  ...
  QueryEntryList=() =>{
    ...
    var Obj = {
      'head': {
        'functionNo': 'plms100037',
        'userNo': userNo,
        'clientTimestamp': loginTokenKey
      },
      'body': {
        'userNo': userNo,
        'cityCode': searchConditions.cityCode,                            //选择城市
        'distributorCode':searchConditions.distributorCode,     //选择渠道
        'customerName': searchConditions.customerName,            //借款人
        'receiptsStartDate': searchConditions.receiptsStartDate, //还款开始日期
        'receiptsEndDate': searchConditions.receiptsEndDate,  //还款结束日期
        "accountTypeCode":searchConditions.accountTypeCode,  //收款方代码
       'fundSourceCode':searchConditions.fundSourceCode,   //还款来源
        'receiptsRecordStatusCode': searchConditions.receiptsRecordStatusCode,//选择状态
  ...
  onSecondOperationClick(item){
    this.router.navigate(['/home/repaymentRegisterQuery', item.receiptsRecordCode]);
  ...
  batchUpload(){
    this.router.navigate(['/home/repaymentRegisterQuery/batch-upload/batchUpload']);
  }
}
