...
//还款计划修改查询列表接口
  ...
  onSecondOperationClick=(item)=>{
    ...
    if(item.canModify=='Y'){
      this.checkUpButton("还款计划修改中","当前借款合同有还款计划修改任务正在处理中，请稍后再查看修改结果。");
    } else if(item.canModify=='N'){
  ...
  checkUpButton(tipTitle, tiptxt){
    ...
    $(this).popModal({
      ...
      cancelButtonClickCallback:myConfirmButtonClickCallback,
    });
