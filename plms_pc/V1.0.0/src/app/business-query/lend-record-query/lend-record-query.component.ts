...
@Component({
  selector: 'app-lend-record-query',
  templateUrl: './lend-record-query.component.html',
  styleUrls: ['./lend-record-query.component.css']
})
export class LendRecordQueryComponent implements OnInit {
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          //绑定成熟数据
          this.add_option(this.cityList, '#cityList', ['cityCode', 'cityName']);
  ...
  getSearchCondigtion() {
    ...
    var paidStartDate = this.commonFunc.handleNilString($("#applyTimeBegin").val()); //还款开始日期
    var paidEndDate = this.commonFunc.handleNilString($("#applyTimeEnd").val());     //还款结束日期
  ...
  QueryEntryList=() =>{
    ...
    var Obj = {
      ...
      'body': {
        ...
        'paidStartDate': searchConditions.paidStartDate,
        'paidEndDate': searchConditions.paidEndDate,
  ...
  //点击修改按钮执行的函数
  linkCheckUpLoan(item){
    //点击查看页面跳转
      this.router.navigate(['/home/loanBusiness', item.contractCode]);
      window.sessionStorage.setItem('tabHighLight','02');
      window.sessionStorage.setItem('backPath','lendRecordQuery');
      this.paidInfoCode=item.paidInfoCode;

  }

  modifyFunc(item){
    this.router.navigate(['/home/lendRecordQuery', item.paidInfoCode])//;
  }

  //点击删除按钮执行的函数
  deleteFunc(item){
    this.ensureButtonClick('删除还款到账',
      '将删除借款人<span id="secondValue">'+item.customerName+'</span>于'+item.creditStartDate+'<span id="secondValue">'+item.principal+'</span>元的资金到账记录，确认删除？',true);
  }
