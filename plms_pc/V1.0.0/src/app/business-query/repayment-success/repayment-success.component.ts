...
import {LoanBusinessDetailComponent} from "../loan-business-detail/loan-business-detail.component";

declare var $: any;
declare var laydate: any;
@Component({
  selector: 'app-repayment-success',
  templateUrl: './repayment-success.component.html',
  styleUrls: ['./repayment-success.component.css'],
  providers:[LoanBusinessDetailComponent]
})

export class RepaymentSuccessComponent implements OnInit {
  ...
  constructor(private networkService: NetworkService,
              ...
              private loanBusinessDetailComponent:LoanBusinessDetailComponent
  ) {
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          var accountTypeList = res.data.body.accountTypeList;//收款賬戶
          //绑定成熟数据
  ...
  getSearchCondigtion() {
    ...
    var accountTypeList = this.commonFunc.handleNilString($("#accountTypeList").val()); //收款方
    var fundSourceCode = this.commonFunc.handleNilString($("#repaymentOrigin").val()); //还款来源
  ...
  QueryEntryList=() =>{
    ...
    var Obj = {
      ...
      'body': {
        ...
        'receiptsStartDate': searchConditions.creditStartDate,
        'receiptsEndDate': searchConditions.creditEndDate,
        'accountTypeCode': searchConditions.accountTypeList,
  ...
  onSecondOperationClick(item){
    ...
    window.sessionStorage.setItem('tabHighLight','07');
    window.sessionStorage.setItem('backPath','repaymentSuccess')
    this.receiptsRecordCode=item.receiptsRecordCode;
  }

  repaymentSuccessClick(code){
    this.router.navigate(['/home/repaymentSuccess', code]);
    // this.loanBusinessDetailComponent.button={
    //   'name':'返回还款查询',
    //   'url':'/home/repaymentSuccess'
    // }
  }
