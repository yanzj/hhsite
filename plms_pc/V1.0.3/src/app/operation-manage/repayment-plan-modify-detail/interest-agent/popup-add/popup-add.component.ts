...
export class PopupAddComponent implements OnInit {
  ...
  constructor(private commonFunc: commonService,
              private user: User,
  ...
  confirmBtn() {
    ...
    var flag = true;
    flag = applyTimeBegin && principal && accruals;
    if (flag) {
      this.addData($("#principal").val(),$("#accruals").val(),$("#applyTimeBegin").val(),$("#guarantee").val(),true);
    }else{
      ...
      var start = {
        ...
        choose: function (datas) {
          if(datas){
            ...
            that.checkData($("#principal").val(),$("#accruals").val(),$("#applyTimeBegin").val(),$("#guarantee").val())
          }
        }
      };
      $("#applyTimeBegin").bind({
...
//  添加数据的接口
  userNo: string;
//在函数内部定义了一个变量  但是放到postData外面输出就是空
  data:any={};
  @Output() event4 = new EventEmitter();
  addData(principal, interest, repaymentDate,guarantee, confirm?) {//传入的参数是本金，利息，还款日期
  ...
  checkData(principal, interest, repaymentDate,guarantee) {//传入的参数是本金，利息，还款日期，
    var loginTokenKey = this.commonFunc.getloginToken();
    var userObj = this.user.getUserData();
    this.userNo = userObj.userNo;
    // var data  = {};
