...
export class ManualRegistrationComponent implements OnInit {
      ...
      var start = {
        ...
        choose: function (datas) {
          ...
          if(datas){
            ...
            // this.manulRegisterInit.creditStartDate 授信起始日期
            // 到账日期大于等于授信日期的校验去掉
            // datas=datas.replace(/-/g,'');
            // var creaditDate=that.manulRegisterInit.creditStartDate.replace(/-/g,'');
            // creaditDate=creaditDate.split(' ');
            // if(datas<creaditDate[0]){
            //   that.Validates.addError("applyTimeBegin", 'bottom', that.tipsMarginLeft, "日期选择错误");
            // }else {
            //   $("#applyTimeBegin").removeTip();
            // }
          }else {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          this.professionalData=[
            ...
            {content:this.manulRegisterInit.contractNo,styleName:'message-number'}
          ]
          //绑定成熟数据
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          // this.getFileList(this.selectItemNameDisplayCode);$('#disableButton').dis
        } else {
          //提交失败 提示用户
        }
  ...
  onSubmitUploadFileFunc() {
    ...
    var applyTimeNull =this.Validates.checkNull("applyTimeBegin", 'bottom', this.tipsMarginLeft, "不能为空");
    //  判断校验是否全部通过
    var validFlag = applyMoneyNull&&receiptNull&&repaymentNull&&applyTimeNull;
    if(!validFlag){
      return false;
    }else {
