...
export class RepaymentPlanModifyDetailComponent implements OnInit {
      ...
      if (res.retCode == true) {
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          if (this.loanBusinessData.paidInfoList.length == 1) {//如果长度是1的话  默认选中
            ...
            $('#recordLending').attr("disabled", true);
  ...
  getModify() {
    ...
    this.networkService.postData(Obj, false).then(res => {
      if (res.retCode == true) {
        ...
        } else if (res.data.head.returnCode == '1009') {
          this.changeModedifyPower('还款计划修改失败', returnMessage, '正在修改当前还款计划，您无法修改提交数据');
        }
        else if (res.data.head.returnCode == '9999') {
  ...
  querySectionFun() {
    this.router.navigate(['/home/repaymentPlanModifyQuery'])
  }
