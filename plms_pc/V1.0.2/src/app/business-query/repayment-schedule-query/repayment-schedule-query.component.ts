...
@Component({
  selector: 'app-repayment-schedule-query',
  templateUrl: './repayment-schedule-query.component.html',
  styleUrls: ['./repayment-schedule-query.component.css']
})
export class RepaymentScheduleQueryComponent implements OnInit {
  ...
  timedicpker() {
    ...
    $.getScript("/assets/js/laydate/laydate.js").then(function () {
      ...
      //日期默认选中及从站内信的消息过来的日期也要默认选中
      var repaymentDate=window.sessionStorage.getItem('repaymentDate')
      if(window.sessionStorage.getItem('sign')=='004'){
        $('#applyTimeBegin').val(laydate.now(0, repaymentDate))
        $('#applyTimeEnd').val(laydate.now(0, repaymentDate))
      }else{
  ...
  ngOnDestroy(){
    ...
    //从消息提醒过来的sesstion销毁
    window.sessionStorage.removeItem('repaymentDate');
    ...
    this.networkService.postData(Obj, false).then(res => {
      ...
      if (res.retCode == true) {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          var payeeList = res.data.body.payeeList;
          //绑定成熟数据
          ...
          this.add_option(loanStatusList, '#applyStatusList', ['loanStatusCode', 'loanStatusName']);
          this.add_option(payeeList, '#receipt', ['payeeCode', 'payeeName']);
  ...
  getSearchCondigtion() {
    ...
    var loanStatusCode = this.commonFunc.handleNilString($("#applyStatusList").val()); //选择状态
  ...
  QueryEntryList=() =>{
    ...
    var Obj = {
      ...
      'body': {
        ...
        'repaymentStartDate': searchConditions.creditStartDate,
        'repaymentEndDate': searchConditions.creditEndDate,
        "agentName": searchConditions.agentName,
        'loanStatusCode': searchConditions.loanStatusCode,
  ...
  onSecondOperationClick(item){
    this.router.navigate(['/home/repaymentSchedule', item.repaymentScheduleCode]);
