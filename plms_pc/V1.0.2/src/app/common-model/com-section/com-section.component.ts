    ...
    this.networkService.postData(Obj, false).then(res => {
        ...
        if (this.networkService.onJudgeSuccessful(res)) {
          ...
          for(let i=0;i<this.messageTipsList.length;i++){
            ...
            var repaymentDate=internalParam.repaymentDate;
            ...
            if(messageMsgType=='001'){  //待评估
              ...
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }else if(messageMsgType=='003'){ //正在评估
              ...
              var messageLink=`<a href="./#/home/repaymentSchedule" class="text-link">${messageKeyWords}</a>`
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }else if(messageMsgType=='005'){ //逾期计划
              var messageLink=`<a href="./#/home/overdueRecord" class="text-link">${messageKeyWords}</a>`
              messageContent=messageContent.replace(messageKeyWords,messageLink)
            }
