...
public class PretreatmentServiceImpl implements PretreatmentService {
    ...
    public Map dispatchServices(Map paramMap) throws Exception {
        ...
        } catch (ErrorException hhBizEx) {
            logger.error("业务异常：", hhBizEx);
            //整理异常情况下的出参
            bodyMap.clear();
            headMap.put(Fields.PARAM_MESSAGE_ERR_CODE, hhBizEx.getErroCode());
            headMap.put(Fields.PARAM_MESSAGE_ERR_MESG, hhBizEx.getMessage() == null || "".equals(hhBizEx.getMessage()) ? ConfigInfo.ERROR_CODE.get(hhBizEx.getErroCode()) : hhBizEx.getMessage());
        } catch (Exception e) {
            logger.error("系统异常：", e);
            //整理异常情况下的出参
            bodyMap.clear();
            headMap.put(Fields.PARAM_MESSAGE_ERR_CODE, ReturnCode.SYSTEM_EXCEPTION.getReturnCode());
