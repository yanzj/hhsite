...
public class HH000905 extends BaseService{
    ...
    public Map<String, Object> subExcute(Map paramMap) throws Exception {
            ...
            for(FmsModelBean fmb : fmsModelBeanList){
                ...
                if(forFileModelBeanList != null && forFileModelBeanList.size() > 0){
                    for(FmsModelBean forFileFmb : forFileModelBeanList){
                        ...
                        forFileMap.put(Fields.PARAM_LOANER, forFileFmb.getPartnerName());
                        ...
                        forFileMap.put(Fields.PARAM_LOAN_CONTRACT_FLAG, "Y".equals(forFileFmb.getLoanContractFlag()) ? Constants.IS_LOAN_CONTRACT : Constants.ISNOT_LOAN_CONTRACT);//01：借款合同 02：非借款合同
                        fileList.add(forFileMap);
                    }
                    groupMap.put(Fields.PARAM_LIST_DTO_ARRAYLIST, fileList);
                }
        ...
        if(bmsReturnMap != null && ReturnCode.SUCCESS_CODE.equals(bmsReturnMap.get(Fields.PARAM_MESSAGE_ERR_CODE))){
            ...
            returnMap.put(Fields.PARAM_MESSAGE_ERR_CODE, bmsReturnMap.get(Fields.PARAM_MESSAGE_ERR_CODE));
            returnMap.put(Fields.PARAM_MESSAGE_ERR_MESG, bmsReturnMap.get(Fields.PARAM_MESSAGE_ERR_MESG));
        }
        return returnMap;
