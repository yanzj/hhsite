...
public class OperationManagerService {
    ...
    public void modifyScheduleDetail(Map paramMap) throws Exception {
        ...
        ((OperationManagerService) AopContext.currentProxy()).updateRepaymentDetailList(rollBachDate,contractCode,batchCode,insertedList,updatedList,deletedList);
    }
     ...
     * @throws Exception
     */
    public void updateRepaymentDetailList(String rollBachDate, String contractCode, String batchCode,List insertedList, List updatedList, List deletedList) throws Exception {
        //修改还款计划表
        ...
        //查找应还总额为0的记录，置为无效
        cancelRepaymentAmountEqualsZeroScheduleDetail(contractCode);
        ...
        for(int i = 0; null!=paidInfoList && i < paidInfoList.size(); i++){
            ...
            for(int p=0; null!=tmpDataList && p < tmpDataList.size(); p++){
                Map result = (Map)tmpDataList.get(p);
                Map tmpMap = new HashMap();
                tmpMap.put("code",result.get("repaymentScheduleDetailCode"));
                tmpMap.put("totalPeriod",tmpDataList.size());
                tmpMap.put("currentPeriod",p+1);
                operationManagerDao.updatePeriodForRepaymentScheduleDetail(tmpMap);
            }
        }
     ...
     */
    void cancelRepaymentAmountEqualsZeroScheduleDetail(String contractCode){
        int upflag = plmsRepaymentScheduleDetailDao.updateScheduleIsUnavaliableWhereAmountIsZero(contractCode);
        upflag = plmsRepaymentScheduleDetailDao.updateScheduleDetailIsUnavaliableWhereAmountIsZero(contractCode);
    }
     ...
     * @throws Exception
     */
    public void rollBackPaymentAndOverdueService(String rollBachDate, String contractCode,String batchCode) throws Exception {
    ...
    public void dealInsertedRepaymentDetailList(List list) throws Exception {
        for(int i=0; i<list.size(); i++){
            ...
            //取账户明细信息
            Account parmAccount = new Account();
            parmAccount.setCode(accountDetail.getAccountCode());
            Account account = accountDao.qryAccount(parmAccount);
            ...
            plmsRepaymentScheduleDetail.setCurrentPeriod(CommonUtil.dealIntegerNull(repaymentDetailMap.get("currentPeriod")));
            plmsRepaymentScheduleDetail.setTotalPeriod(CommonUtil.dealIntegerNull(repaymentDetailMap.get("totalPeriod")));
            plmsRepaymentScheduleDetail.setRepaymentDate(DateUtil.parseDate(repaymentDate));
            plmsRepaymentScheduleDetail.setAmount(CommonUtil.dealBigDecimalNull2Zero(repaymentDetailMap.get("amount").toString()));
            ...
            plmsRepaymentScheduleDetail.setPrincipalAccountCode(CommonUtil.dealStringNull(repaymentDetailMap.get("principalAccountCode")));
            plmsRepaymentScheduleDetail.setInterestAccountCode(CommonUtil.dealStringNull(repaymentDetailMap.get("interestAccountCode")));
            ...
            accountDetail.setRemainingQuota(MathUtil.strSub(accountDetail.getTotalQuota(),accountDetail.getPrincipal(),GlobParam.SCALE_2).toString());//授信金额 – 已贷本金
            ...
            account.setRemainingQuota(MathUtil.strSub(account.getTotalQuota(),account.getPrincipal(),GlobParam.SCALE_2).toString());//授信金额 – 已贷本金
