...
public class PayScheduleDetailForContractImpl implements PayScheduleDetailForContract {
    ...
    @Resource
    RepaymentDetailTempDao repaymentDetailTempDao;

    /**
     * 根据批次号和业务发生日期扣款(用于重新计算扣款)
     * @param contractCode
     * @param happenTime
     * @return
     * @throws Exception
    ...
    public boolean payContractByBatchCode(String contractCode, Date happenTime, String batchCode,String paymentMethod) throws Exception {
        if(null == batchCode || null == happenTime){
            throw new ErrorException(ReturnCode.REQUIRED_FIELD_MISSING, "批次号或者发生时间为空");
        }
        ...
        batchMap.put("batchCode",batchCode);
        batchMap.put("timeHappened",happenTime);
        batchMap.put("paymentMethod",paymentMethod);

        List scheduleDetailList = repaymentDetailTempDao.qryScheduleDetailByBatchCodeHappenTimeAndPayMethod(batchMap);
        Map requestMap = new HashedMap();
        ArrayList<Map> detailList = new ArrayList();
        for (int i = 0 ; i < scheduleDetailList.size();i++){
            Map detail = (HashMap)scheduleDetailList.get(i);
            String scheduleDetailCode = (String)detail.get("detailCode");
            batchMap.put("scheduleDetailCode",scheduleDetailCode);
            List plmsRepaymentDetailTempList = repaymentDetailTempDao.findRepayDtlTmpByBatchCodeHappenTimeAndPayMethod(batchMap);
            boolean hasBailPenalty = false;
            boolean hasServiceFeePenalty = false;
            boolean hasInterestPenalty = false;
            boolean hasPrincipalPenalty = false;
            boolean hasBail = false;
            boolean hasServiveFee = false;
            boolean hasInterest = false;
            boolean hasPrincipal = false;
            for (int j = 0; j < plmsRepaymentDetailTempList.size();j++){
                Map plmsRepaymentDetailTemp = (Map)plmsRepaymentDetailTempList.get(i);
                Integer subjectInt = (Integer)plmsRepaymentDetailTemp.get("subject");
                String subject = "";
                if (subjectInt != null){
                    subject = subjectInt.toString();
                }
                if (subject != null && GlobDict.repayment_detail_subject_prepayment_penalty.getKey().equals(subject)){
                    hasBailPenalty = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_service_fee_penalty.getKey().equals(subject)){
                    hasServiceFeePenalty = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_interest_overdue.getKey().equals(subject)){
                    hasInterestPenalty = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_principal_overdue.getKey().equals(subject)){
                    hasPrincipalPenalty = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_bail.getKey().equals(subject)){
                    hasBail = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_service_fee.getKey().equals(subject)){
                    hasServiveFee = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_interest.getKey().equals(subject)){
                    hasInterest = true;
                }
                if (subject != null && GlobDict.repayment_detail_subject_principal.getKey().equals(subject)){
                    hasPrincipal = true;
                }
            }
            ...
            if(null == overdueList || overdueList.size() == 0){
                ...
                for(PlmsOverdue overdue: overdueList){
                    ...
                    if(hasServiceFeePenalty&&GlobDict.overdue_subject_fee.getKey().equals(subject)){
                        //应还服务费罚息
                    ...
                    } else if(hasInterestPenalty&&GlobDict.overdue_subject_interest.getKey().equals(subject)){
                        //应还利息罚息
                    ...
                    }else if(hasPrincipalPenalty&&GlobDict.overdue_subject_principal.getKey().equals(subject)){
                        //应还本金罚息
                    ...
                    }else if(hasBailPenalty&&GlobDict.overdue_subject_bail.getKey().equals(subject)){
                        //应还保证金罚息
            ...
            if(isInterestInstead.equals(GlobDict.apply_interesting_is_interest_Instead_no.getKey())){
                ...
                //判断是否已还
                if(hasServiveFee&&bigServiceFee.compareTo(bigRepaymentedServiceFee) != 0){
            ...
            //判断是否已还
            if(hasInterest&&bigInterest.compareTo(bigRepaymentedInterest) != 0){
            ...
            //判断是否已还
            if(hasBail&&bigBail.compareTo(bigRepaymentedBail) != 0){
                Map interestMap = new HashedMap();
            ...
            //判断是否已还
            if(hasPrincipal&&bigPrincipal.compareTo(bigRepaymentedPrincipal) != 0){
...
//        requestMap.put("schedualDetailCode",schedualDetailCode);
...
//        for(Map detail: scheduleDetailList){
        ...
        if(detailList.size() > 0){
            ...
            for(int i = 0; i < detailList.size(); i++){
                ...
                plmsRepaymentDetail.setPaymentMethod(GlobDict.payment_method_manual.getKey());
    ...
    public boolean payContract(String contractCode, Date happenTime) throws Exception {
        ...
        if(detailList.size() > 0){
            ...
            for(int i = 0; i < detailList.size(); i++){
                ...
                plmsRepaymentDetail.setSubject(subject);
                plmsRepaymentDetail.setScheduleDetailCode(detailCode);
                plmsRepaymentDetail.setPaymentMethod(GlobDict.payment_method_auto.getKey());
                plmsRepaymentDetail.setStatus(GlobDict.repayment_detail_status_avaliable.getKey());
                plmsRepaymentDetail.setTimeHappened(happenTime);
                plmsRepaymentDetail.setCreateDate(happenTime);
                plmsRepaymentDetail.setModifyDate(happenTime);
