 ...
 */
@Service
public class Plms100023 extends BaseService {

    private static final Logger logger = Logger.getLogger(Plms100023.class);
    ...
    public void busiService(Map<String, Object> head, Map<String, Object> body, Map<String, Object> resultMap) throws ErrorException, Exception {
        ...
        if(null != body.get("repaymentDate") && !"".equals(body.get("repaymentDate").toString().trim())){
            repaymentDate = body.get("repaymentDate").toString();
        }

        String principal = "0";
        if(null != body.get("principal") && !"".equals(body.get("principal").toString().trim())){
            principal = body.get("principal").toString();
        }

        String interest = "0";
        if(null != body.get("interest") && !"".equals(body.get("interest").toString().trim())){
            interest = body.get("interest").toString();
        }

        String serviceFee = "0";
        if(null != body.get("serviceFee") && !"".equals(body.get("serviceFee").toString().trim())){
            serviceFee = body.get("serviceFee").toString();
        }

        String bail = "0";
        if(null != body.get("bail") && !"".equals(body.get("bail").toString().trim())){
            serviceFee = body.get("bail").toString();
        }
        ...
        param.put("principal",new BigDecimal(principal));
        param.put("interest",new BigDecimal(interest));
        param.put("serviceFee",new BigDecimal(serviceFee));
        param.put("bail",new BigDecimal(bail));
        operationManagerDao.createOneRepaymentScheduleDetailTmpByPaidCode(param);
