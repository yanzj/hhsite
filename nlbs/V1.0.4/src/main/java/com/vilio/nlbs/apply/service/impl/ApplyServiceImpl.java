...
public class ApplyServiceImpl implements ApplyService {
        ...
        if (ReturnCode.SUCCESS_CODE.equals(bmsResultMap.get(Fields.PARAM_MESSAGE_ERR_CODE))) {
            ...
            if (bmsPeopleList != null) {
                for (Map bmsPeopleMap : bmsPeopleList) {
                    ...
                    Object cellPhone = bmsPeopleMap.get("cellphone");
                    String telPhone = cellPhone == null ? "" : cellPhone.toString().replaceAll("(\\d{3})\\d{4}\\d{4}","$1********");
                    peopleMap.put(Fields.PARAM_PERSON_MOBILEPHONE, telPhone);
            ...
            if (bmsMortgageList != null) {
                ...
                for (Map bmsMortgageMap : bmsMortgageList) {
                    ...
                    String propertyOwners = bmsMortgageMap.get("propertyOwner") == null ? "" : bmsMortgageMap.get("propertyOwner").toString();
                    mortgageMap.put(Fields.PARAM_MORTGAGE_PROPERTY_OWNERS, propertyOwners.replaceAll(";", "、"));
                    ...
                    mortgageMap.put(Fields.PARAM_INVESTINF_SECOND_MORTGAGE_PRICE, bmsMortgageMap.get("mortgageSecondAmount"));
            ...
            //Step 3.7 放款账户信息
            if (bmsResultMap.get("accountNumber") != null && bmsResultMap.get("accountNumber") != null && bmsResultMap.get("accountNumber") != null) {
                Map loanAccountMap = new HashMap();
                loanAccountMap.put(Fields.PARAM_LOAN_ACCOUNT, bmsResultMap.get("accountNumber"));
                loanAccountMap.put(Fields.PARAM_LOAN_ACCOUNT_NAME, bmsResultMap.get("accountName"));
                loanAccountMap.put(Fields.PARAM_LOAN_ACCOUNT_BANK, bmsResultMap.get("openingBank"));
                        ...
                        if (fmsBodyReturnMap != null) {
                            ...
                            if (fmsFileList != null && fmsFileList.size() > 0) {
                                for(Map fmsFileMap : fmsFileList) {
                                    ...
                                    Object fmsFileSuffix = fmsFileMap.get("fileSuffix");
                                    ...
                                    fileMap.put("fileSuffix", fmsFileSuffix);
                                    for (Map bmsPeratingArchiveMap : bmsPeratingArchiveList) {
                                        ...
                                        fileMap.put(Fields.PARAM_FILE_NAME, fileName);//重写文件名称，取核心系统的文件名
            ...
            if (subOperationList != null && subOperationList.size() > 0) {
                ...
                for(Map subOperationMap : subOperationList){
                    if(subOperationMap.get("lendingAcount") != null && subOperationMap.get("lendingAcountName") != null && subOperationMap.get("lendingBank") != null){
                                            ...
                                            if (fileType.equals(bmsFileType)) {
                                                for (Map fmsFileMap : fmsFileList) {
                                                    if (fmsFileMap.get(Fields.PARAM_SERIAL_NO).equals(bmsFileId)) {
                                                        ...
                                                        fileMap.put(Fields.PARAM_ORIGINAL_FILE_NAME, bmsFileName);//取核心系统的文件名称
                                                        ...
                                                        fileMap.put("fileSuffix", fmsFileMap.get("fileSuffix"));
    ...
    public Map getFile(Map map) throws Exception {
        ...
        try {
            ...
            in = HttpRequestUtils.httpGetFile(url, null);
            result.put("is", in);
        } catch (Exception e) {
    ...
    public Map downloadFile(Map map, HttpServletResponse rsp) throws Exception {
        Map result = new HashMap();
        String serialNo = (String) map.get("serialNo");
