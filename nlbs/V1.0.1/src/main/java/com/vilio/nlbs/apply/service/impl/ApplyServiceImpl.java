...
public class ApplyServiceImpl implements ApplyService {
    ...
    @Resource
    NlbsRecordClerkDistributorMapper nlbsRecordClerkDistributorMapper;
    ...
    @Resource
    NlbsAgentMapper nlbsAgentMapper;
    ...
    public Map initApplyInfo(Map paramMap) throws Exception {
        ...
        List<NlbsDistributor> distributorList = (List<NlbsDistributor>) returnMap.get(Fields.PARAM_DISTRIBUTRO_LIST);
        if (null != returnMap && null != distributorList && distributorList.size() > 0) {
            for (NlbsDistributor nb : distributorList) {
                Map nbMap = new HashMap();
                nbMap.put(Fields.PARAM_DISTRIBUTRO_CODE, nb.getCode());
                nbMap.put(Fields.PARAM_DISTRIBUTRO_NAME, nb.getAbbrName());
    ...
    public Map getAgentAndProductList(Map paramMap) throws Exception {
        ...
        List<NlbsAgent> nlbsAgentList = nlbsAgentMapper.selectAgentByDistributorCode(distributorCode);
        if(nlbsAgentList != null){
            for(NlbsAgent na: nlbsAgentList){
                Map nlbsAgentMap = new HashMap();
                nlbsAgentMap.put(Fields.PARAM_AGENT_ID, na.getAgentId());
                nlbsAgentMap.put(Fields.PARAM_AGENT_NAME, na.getName());
    ...
    public Map initApplyList(Map paramMap) throws Exception {
        ...
        if (null != returnMap && null != distributorList && distributorList.size() > 0) {
            ...
            for(NlbsDistributor distributor: distributorList){
                String cityCode = distributor.getCityCode();
    ...
    private Map getAvaliableDistributorList(String userNo) throws Exception {
        ...
        if (nlbsRoleUserList != null) {
            for (NlbsRoleUser nr : nlbsRoleUserList) {
                if (Constants.ROLE_SUPPER_MANAGER.equals(nr.getRoleCode()) || Constants.ROLE_SYSTEM_MANAGER.equals(nr.getRoleCode())
                        || Constants.ROLE_BUSINESS_MANAGER.equals(nr.getRoleCode())) {
                    role =  MANAGER_FLAG;
                    beAdminFlag =  true;
                }
            }
        ...
        if(!beAdminFlag){
            //如果不是管理员，看是否是录单员
            if(null != nlbsUser){
                if ("Y".equals(nlbsUser.getBeRecordClerk())) {
                    role =  RECORD_FLAG;
                    beAdminFlag =  true;
                } else {
                    role =  AGENT_FLAG;
        ...
        List<NlbsDistributor> returnDistributorList = new ArrayList<NlbsDistributor>();
        if (MANAGER_FLAG == role) {
            returnMap.put(Fields.PARAM_BE_RECORD_CLERK, "Y");
            //Step 2 获取进件公司（渠道）列表
            //Step 2.1 管理员，则获取全部渠道
            List<NlbsDistributor> nlbsDistributorList = nlbsDistributorMapper.selectAll();
            returnDistributorList.addAll(nlbsDistributorList);
            ...
            if(null != nlbsDistributor){
                returnDistributorList.add(nlbsDistributor);
            }
            ...
            //Step 2.3.1先查找关联表的数据
            List<NlbsDistributor> agentDistributorList = nlbsDistributorMapper.selectAgentDistributorListByAgentId(nlbsUser.getAgentId());
            if(agentDistributorList != null){
                nlbsDistributorList.addAll(agentDistributorList);
                //Step 2.3.2再循环查找子渠道
                for(NlbsDistributor nb : agentDistributorList){
                    nlbsDistributorList.addAll(commonService.selectChildrenListDistributor(nb.getCode()));
                }
            }
            //Step 2.3.3 整理distributerList的参数
            if(nlbsDistributorList != null){
                returnDistributorList.addAll(nlbsDistributorList);
            }
        }
