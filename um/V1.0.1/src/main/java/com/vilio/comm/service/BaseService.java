...
public class BaseService {
    ...
    public void checkField(String field, String name, Integer needLength, Integer maxLength) throws ErrorException {
        if (field == null || field.length() == 0) {
            throw new ErrorException("9998", String.format("缺少%s必要参数,或数据格式不正确", name));
        }
        if (needLength != null) {
            if (field == null || field.length() != needLength) {
                throw new ErrorException("9998", String.format("%s长度必须为%d", name, needLength));
            }
        }
        if (maxLength != null) {
            if (field == null || field.length() > maxLength) {
                throw new ErrorException("9998", String.format("%s长度不能大于%d", name, maxLength));
            }
        }
    }
