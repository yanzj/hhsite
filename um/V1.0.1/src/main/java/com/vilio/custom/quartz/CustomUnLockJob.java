...
public class CustomUnLockJob extends QuartzJobBean {
    ...
    public void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        try {
            ...
            logger.info("custom登录解锁定时任务结束");


            logger.info("custom交易解锁定时任务开始");
            String transUnLockTime = GlobParam.customTransLockTime;
            updateParam = new HashMap();
            updateParam.put("transUnHashLock", GlobDict.trans_un_hash_lock.getKey());
            updateParam.put("transHashLock", GlobDict.trans_hash_lock.getKey());
            updateParam.put("systemTimestamp", String.valueOf(System.currentTimeMillis()));
            updateParam.put("transUnLockTime", transUnLockTime);
            updateParam.put("transError", 0);
            //交易锁定的用户超过阈值解锁
            ret = customUserDao.updateTransUnLock(updateParam);
            if (ret>0){
                logger.info("交易解锁用户数："+ret);
            }else if (ret == 0 ){
                logger.info("交易无锁定用户"+ret);
            }else{
                logger.error("交易解锁报错："+ret);
            }

            logger.info("custom交易解锁定时任务结束");
