...
public class C200006 extends BaseService {
    ...
    public void busiService(Map<String, Object> head, Map<String, Object> body, Map<String, Object> resultMap) throws ErrorException, IllegalAccessException, IntrospectionException, InvocationTargetException {
        ...
        //定义修改用户参数
        user.setMobile(body.get("mobile") == null ? user.getMobile() : body.get("mobile").toString());
        user.setEmail(body.get("email") == null ? user.getEmail() : body.get("email").toString());
        user.setFullName(body.get("fullName") == null ? user.getFullName() : body.get("fullName").toString());
        user.setSex(body.get("sex") == null ? user.getSex() : body.get("sex").toString());
        user.setBirthday(body.get("birthday") == null ? user.getBirthday() : body.get("birthday").toString());
        user.setBirthAddr(body.get("birthAddr") == null ? user.getBirthAddr() : body.get("birthAddr").toString());
        user.setNick(body.get("nick") == null ? user.getNick() : body.get("nick").toString());
        user.setHeadImg(body.get("headImg") == null ? user.getHeadImg() : body.get("headImg").toString());
